...
lsb_release -a
```
## Если нужно обновить версию Ubuntu 22.04 до 24.0422, можно это выполнить принудительный переход через смену репозиториев (надёжно)
⚠️ Выполняй только если это чистая Ubuntu 22.04, не кастомизированная. Перед обновлением в файле /etc/update-manager/release-upgrades установить Prompt=normal
```bash. 💡 Рекомендация перед апгрейдом, Сделай резерв:
```bash
