...
## 👩‍💻 Этот гайд поможет развернуть Hyperliquid Testnet на Ubuntu 24.04 LTS.
---
### 📗 Содержание 🌱
...
lsb_release -a
```
#### Если нужно обновить версию Ubuntu 22.04 до 24.04, можно выполнить принудительный переход через смену репозиториев (надёжно)
...
```
#### Во время установки нужно будет ответить на несколько вопросов системы:
...
---
### 📥 Установка HL-Visor 💚
1. Импорт публичного ключа Hyperliquid (этот ключ нужен только для проверки целостности и подлинности бинарника):
```bash
curl -O https://raw.githubusercontent.com/hyperliquid-dex/node/main/pub_key.asc
gpg --import pub_key.asc
```
2. Установка бинарника hl-visor и проверка подписи:
```bash
...
```
Если Если вывод похож на что-то подобное, то бинарник готов к использованию.
```bash
...
```
3. Делаем его исполняемым и проверяем версию:
```bash
...
```
Если ответ "hyperliquid-visor 691d819be5d5e2b0f912023f992ad392b808c1e9|2025-11-03 15:06:27 +0800|uncommitted=false" значит hl-visor стал исполняемым.🎉
После удаляем лишнее:
```bash
...
```bash
cd ~
./hl-visor run-non-validator --write-trades --write-order-statuses --serve-evm-rpc
```
...
```
---
### 🌐 Validator (дополнительно)
Если вы планируете запускать Validator, выполните следующие шаги:
Настройте validator_config.json и добавьте свои ключи и запустите ноду в режиме валидатора:
1. 1️⃣ Генерация ключа для валидатора
```bash
...
2. Запуск в валидаторском режиме (вручную, чтобы не путать systemd).
```bash
cd /root/hl
...
journalctl -u hl-visor | grep "connected to abci stream from" | awk '{print $NF}' | sort | uniq | wc -l
```
2. Управление сервисом hl-visor (запуск, остановка, перезапуск, статус и логи)
```bash 
